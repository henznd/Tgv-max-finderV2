-- ============================================================
-- CONFIGURATION COMPLÈTE : Collecte de prix toutes les secondes
-- ============================================================
-- À exécuter dans l'éditeur SQL de Supabase
-- https://jlqdkbdmjuqjqhesxvjg.supabase.co → SQL Editor

-- ============================================================
-- ÉTAPE 1: Créer la table price_history
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_price_history_token_timestamp ON price_history(token, timestamp DESC);
...
DROP POLICY IF EXISTS "Allow authenticated insert" ON price_history;
CREATE POLICY "Allow authenticated insert" ON price_history
    FOR INSERT
    WITH CHECK (true);
...
-- ============================================================
-- ÉTAPE 3: Fonction pour collecter les prix ETH
-- ============================================================

CREATE OR REPLACE FUNCTION collect_prices_direct()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Collecter les prix depuis Lighter (ETH market_id = 0)
  BEGIN
    SELECT * INTO lighter_result
    FROM net.http_get(
      url := 'https://mainnet.zklighter.elliot.ai/api/v1/orderBookOrders?market_id=0&limit=1',
    ...
    IF lighter_result.status_code = 200 AND lighter_result.content IS NOT NULL THEN
      ...
      BEGIN
        ...
        IF lighter_data->'asks' IS NOT NULL AND 
          ...
          INSERT INTO price_history (timestamp, token, exchange, bid, ask, mid)
          VALUES (
            timestamp_now,
            'ETH',
            'lighter',
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'Erreur parsing Lighter: %', SQLERRM;
      END;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Erreur collecte Lighter: %', SQLERRM;
  END;
  
  -- Collecter les prix depuis Paradex (ETH-USD-PERP)
  BEGIN
    SELECT * INTO paradex_result
    FROM net.http_get(
      url := 'https://api.prod.paradex.trade/v1/orderbook/ETH-USD-PERP',
      headers := '{"Content-Type": "application/json"}'::jsonb
    );
    ...
    IF paradex_result.status_code = 200 AND paradex_result.content IS NOT NULL THEN
      ...
      BEGIN
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'Erreur parsing Paradex: %', SQLERRM;
      END;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Erreur collecte Paradex: %', SQLERRM;
  END;
END;
$$;

-- ============================================================
-- ÉTAPE 4: Fonction qui collecte en boucle (60 fois par minute)
-- ============================================================
...
BEGIN
  ...
  WHILE now() < end_time LOOP
    PERFORM collect_prices_direct();
    PERFORM pg_sleep(1.0);  -- Attendre 1 seconde
    i := i + 1;
  END LOOP;
  
  RAISE NOTICE 'Collecté % fois en 1 minute', i;
END;
$$;

-- ============================================================
-- ÉTAPE 5: Créer le job cron pour collecter toutes les minutes
-- ============================================================

-- Supprimer le job existant s'il existe
SELECT cron.unschedule('collect-prices-every-second') WHERE EXISTS (
...
-- ============================================================
-- ÉTAPE 6: Vérifier que tout est configuré
-- ============================================================
...
BEGIN
  ...
  RAISE NOTICE '   La collecte de prix démarre automatiquement';
  RAISE NOTICE '   Les prix seront collectés toutes les secondes';
  RAISE NOTICE '   Vérifiez avec: SELECT * FROM price_history ORDER BY timestamp DESC LIMIT 10;';
END $$;
...
-- Pour tester manuellement la collecte:
-- SELECT collect_prices_direct();
...
-- Pour redémarrer la collecte:
-- SELECT cron.schedule('collect-prices-every-second', '* * * * *', $$SELECT collect_prices_loop_minute();$$);

-- Pour voir les dernières collectes:
-- SELECT * FROM price_history ORDER BY timestamp DESC LIMIT 20;
